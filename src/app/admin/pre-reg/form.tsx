    ...
    <form onSubmit={submit} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-2 lg:grid-cols-4">
        <label className="flex flex-col text-xs font-medium text-slate-700">
          ...
          <input name="regNumber" required placeholder="AB12 CDE" className="mt-1 rounded-lg border border-slate-300 px-3 py-1.5 font-mono text-sm uppercase" />
        </label>
        <label className="flex flex-col text-xs font-medium text-slate-700">
        ...
        <label className="flex flex-col text-xs font-medium text-slate-700">
          VIN <span className="font-normal text-slate-400">(optional)</span>
          <input name="vin" placeholder="WF0AXX…" className="mt-1 rounded-lg border border-slate-300 px-3 py-1.5 font-mono text-sm uppercase" />
        </label>
        <label className="flex flex-col text-xs font-medium text-slate-700">
      ...
      <div className="mt-4 flex items-center gap-3">
        <button type="submit" disabled={pending} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-50">
          {pending ? "Adding…" : "Add pre-registered vehicle"}
        </button>
        {error && <span className="text-xs text-red-600">{error}</span>}
        {saved && !error && <span className="text-xs text-emerald-700">Added. It&apos;s on both stock lists now.</span>}
      </div>
    </form>
  );
}
