...
export default async function BrokerNewVanSearchPage() {
  const me = await requireBrokerUser();
  const { rows, latestUploadedAt } = await loadMappedStock();
  const vans = rows.filter((r) => isVanBucket(r.bucket));
  const broker: BrokerRow[] = vans.map((r) => ({
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <BrokerHeader me={me} pathname="/broker/search/new-van" />
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-10">
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">Search a new van</h1>
        <p className="mt-1 text-sm text-slate-500">
          {broker.length.toLocaleString()} commercial vehicles in stock.
          {latestUploadedAt && (
