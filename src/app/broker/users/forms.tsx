...
import {
  ...
  resetTeamUserSetupTokenAction,
  setTeamUserActiveAction,
...
function SetupLinkBanner({ path, expiresAt, onDismiss }: { path: string; expiresAt: string; onDismiss: () => void }) {
  ...
  return (
    <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs">
      ...
      <div className="mt-1 text-emerald-900/80">Share this — it expires {new Date(expiresAt).toLocaleString("en-GB")}.</div>
...
export function AddTeamUserForm() {
  ...
  function submit(e: React.FormEvent) {
    ...
    setError(null);
    start(async () => {
      const res = await createTeamUserAction({ name: name.trim(), email: email.trim(), role });
...
export function TeamUsersTable({ currentUserId, users }: { currentUserId: string; users: TeamRow[] }) {
  const router = useRouter();
  const [pending, start] = useTransition();
  const [link, setLink] = useState<{ path: string; expiresAt: string } | null>(null);

  function toggleActive(u: TeamRow) {
    if (u.id === currentUserId) return;
    start(async () => {
      const res = await setTeamUserActiveAction(u.id, !u.active);
      if (res.ok) router.refresh();
    });
  }
  function toggleRole(u: TeamRow) {
    if (u.id === currentUserId && u.role === "owner") return;
    start(async () => {
      const res = await setTeamUserRoleAction(u.id, u.role === "owner" ? "user" : "owner");
      if (res.ok) router.refresh();
    });
  }
  function regenerate(u: TeamRow) {
    start(async () => {
      const res = await resetTeamUserSetupTokenAction(u.id);
      if (res.ok) { setLink({ path: res.setupPath, expiresAt: res.expiresAt }); router.refresh(); }
    });
  }
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
              ...
              return (
                <tr key={u.id}>
                  <td className="px-4 py-3">
                    <div className="font-medium text-slate-900">{u.name} {isSelf && <span className="text-[10px] uppercase tracking-wide text-slate-400">(you)</span>}</div>
                    <div className="text-xs text-slate-500">{u.email}</div>
                  </td>
                  <td className="px-4 py-3">
                    <button
                      onClick={() => toggleRole(u)}
                      disabled={pending || (isSelf && u.role === "owner")}
                  ...
                  <td className="px-4 py-3 text-right space-x-3 whitespace-nowrap">
                    ...
                    <button onClick={() => toggleActive(u)} disabled={pending || isSelf} className="text-xs text-slate-600 hover:text-slate-900 disabled:opacity-50">
            ...
            {users.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-6 text-center text-sm text-slate-500">No team yet — add one above.</td></tr>
            )}
          </tbody>
        </table>
      </div>
